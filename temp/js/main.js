...
localDataName = "subscriptionInformation"
localSortPreferenceName = "sortPreference"
localHasChangedOrder = "hasChangedOrder"
...
// Calculate whether the button should say top or bottom
window.onscroll = () => {
  ...
  if (pctScrolled > 50){
    document.querySelector('#myBtn').innerHTML = "Top";
    scrollDistance = 0
  }
  else{
    document.querySelector('#myBtn').innerHTML = "Bottom"
    scrollDistance = trackLength
  }
}

// Returns true if valid subscription, false if not.
let isValidSubscription = (subscription) => {
  if (isNaN(subscription.amount)) {
  ...
  Swal.fire({
    ...
    preConfirm: () => {
      ...
      let newSub = {
        ...
        currency: document.getElementById('swal-input3').value,
        months: document.getElementById('swal-input4').value,
        notes: document.getElementById('swal-input5').value,
        changed: false
      }

      // Do some form validation
      let valid = isValidSubscription(newSub)
...
let editSubscription = (btn) => {
  ...
  Swal.fire({
    ...
    html:
      ...
      '<label class="label has-text-left">Amount</label> ' +
      '<input id="swal-input2" class="swal2-input" placeholder="Original Amount" value="' + currentSubscription.amount + '">' +
      '<label class="label has-text-left">Currency</label> ' +
...
let addCurrency = () => {
  ...
  Swal.fire({
    ...
    preConfirm: () => {
      ...
      let newCurrency = {
        symbol: document.getElementById('swal-input1').value,
...
let populateTable = (tbody) => {
  ...
  // calculate the monthly cost in dkk of each item, and add it to the running total
  for (elem of subscriptions) {
    elem.monthly = elem.amount / elem.months
    elem.indkk = elem.monthly * currencies.get(elem.currency)
    total += elem.indkk
  }

  // Update tablefoot with total monthly cost and amount of subscriptions.
  document.querySelector('#total').innerHTML = total.toFixed(2)
  document.querySelector('#amountOfSubscriptions').innerHTML = subscriptions.length
  ...
  for (let i = 0; i < subscriptions.length; i++) {
    ...
    tr.innerHTML = '<td>' + elem.title + '</td>' +
      ...
      '<td>' + `<button class="button" onclick="moveRow(this, -1)">Up</button>` + `<button class="button" onclick="moveRow(this, +1)">Down</button>` + `<button class="button" onclick="editSubscription(this)">Edit</button>` + '</td>';
...
let getInfoOfSubscription = (btn) => {
  ...
  Swal.fire(
    currentSub.title,
    `${currentSub.amount}${currentSub.currency} over ${currentSub.months} months, which equates to <br>
...
// Moves a row by some direction (+x, -x)
let moveRow = (btn, direction) => {

  // Find the subscriptions current index
  let currentSub = getSubscriptionFromButton(btn)
  let indexOfCurrentRow = subscriptions.indexOf(currentSub)

  // If trying to move a subscription out of bounds
  if (indexOfCurrentRow + direction < 0 || indexOfCurrentRow + direction >= subscriptions.length) {
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: 'Cant move this object ' + (direction == -1 ? "up" : "down"),
    })
  } else {
    // Move the subscription
    subscriptions[indexOfCurrentRow] = subscriptions[indexOfCurrentRow + direction]
    subscriptions[indexOfCurrentRow + direction] = currentSub
    currentSub.changed = true
    updateTable()

    // Store that the user has a custom order
    setStored(localHasChangedOrder, true)
  }
}
...
let deleteSubscription = (subscription) => {
  subscriptions = subscriptions.filter(s => s.title != subscription.title)
  updateTable()
}

// Make subscriptions and currency ready to be stored as JSON
let getSaveInfo = () => {
  return {subscriptions: subscriptions, keys: [...currencies.keys()], values: [...currencies.values()]}
}

// Parse a save object, and use it as the current info.
let loadSaveInfo = (loadedSave) => {
  subscriptions = loadedSave.subscriptions

  currencies.clear()
  for (let i = 0; i < loadedSave.keys.length; i++) {
    currencies.set(loadedSave.keys[i], loadedSave.values[i])
  }
}
...
let load = () => {
  ...
  function receivedText(e) {
    ...
    loadedSave = JSON.parse(lines);
    loadSaveInfo(loadedSave)
...
try {
  loadSaveInfo(JSON.parse(getStored(localDataName)))

  // Find out if we want to sort the array of the users data.
  if (!getStored(localHasChangedOrder)){
