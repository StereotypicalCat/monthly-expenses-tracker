...
let isValidNewSubscription = (subscription) => {
...
let createNewSubscription = (title, price, currency, months, notes) => {
    const newSub = {
        changed: false,
        currency: currency,
        indkk: 0,
...
let newSubscription = () => {
    ...
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            const currency = document.getElementById('swal-input3').value;
            const months = parseInt(document.getElementById('swal-input4').value);
            const notes = document.getElementById('swal-input5').value;
...
let editSubscription = (btn) => {
    ...
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            let valid = isValidSubscription(editedSub);
            console.log(valid);
            if (valid) {
...
let addCurrency = () => {
    ...
    Swal.fire({
        ...
        html: '<input id="swal-input1" class="swal2-input" placeholder="Currency Symbol">' +
        ...
        preConfirm: () => {
            ...
            let newCurrency = {
                ...
                rate: parseFloat(document.getElementById('swal-input2').value),
            };
...
let Reset = () => {
    ...
    Swal.fire({
        ...
        if (result.value) {
            ...
            updateTable();
            // @ts-ignore
            Swal.fire('Reset!', 'Your subscriptions have been reset.', 'success');
        }
    });
};
...
let getInfoOfSubscription = (btn) => {
    ...
    // @ts-ignore
    Swal.fire(currentSub.title, `${currentSub.price}${currentSub.currency} over ${currentSub.months} months, which equates to <br>
...
let loadSaveInfo = (loadedSave, oldLoadedSave) => {
    let actualSaveToUse = loadedSave;
    if (oldLoadedSave !== null) {
        // Old data
        console.log("loading legacy data...");
        const oldData = oldLoadedSave;
        let newData = { keys: [], subscriptions: [], values: [], version: 0 };
        newData.keys = oldData.keys;
        newData.version = 1;
        newData.values = oldData.values;
        for (let sub of oldLoadedSave.subscriptions) {
            const newSub = createNewSubscription(sub.title, sub.amount, sub.currency, sub.months, sub.notes);
